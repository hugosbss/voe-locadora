...
@section('title', 'Segurança &middot; Painel da Locadora')

@section('content')
    <div class="space-y-6">
        <div>
            <h1 class="text-lg font-semibold tracking-tight text-slate-900">Segurança</h1>
            <p class="mt-0.5 text-sm text-slate-500">Visão geral e proteção da sua conta.</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            @foreach ([
                ...
                <div class="card p-4">
                    <p class="text-2xl font-semibold tracking-tight text-slate-900">{{ number_format($value) }}</p>
        ...
        <div class="card p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 class="text-sm font-semibold text-slate-900">Verificação em duas etapas (2FA)</h2>
                ...
                @else
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200">
                        Desativado
                    </span>
                @endif
            </div>
                    ...
                    @else
                        <div x-data="{ showSecret: false }" class="space-y-5">
                            <div class="flex flex-col gap-5 sm:flex-row">
                                <div class="flex-1">
                                    <h3 class="mb-2 text-sm font-medium text-slate-900">Passo 1 — Cadastre no aplicativo</h3>
                                    <div class="flex items-start gap-4">
                                        <img src="{{ route('admin.security.2fa.qr') }}" alt="QR code para cadastro no autenticador"
                                            class="h-44 w-44 rounded-xl ring-1 ring-slate-200" width="220" height="220">
                                        <div class="text-xs leading-relaxed text-slate-600">
                                            ...
                                            <details class="mt-3">
                                                <summary class="cursor-pointer text-indigo-600 underline-offset-2 hover:underline">Mostrar chave manual</summary>
                                                <code class="mt-2 block break-all rounded-lg bg-slate-50 px-3 py-2 text-xs ring-1 ring-slate-200">
                                                    {{ $enrollment['secret'] }}
                                                </code>
                                            </details>
                                        </div>
                                ...
                                <div class="flex-1">
                                    <h3 class="mb-2 text-sm font-medium text-slate-900">Passo 2 — Guarde os códigos de recuperação</h3>
                                    @if (count($enrollment['recovery_codes'] ?? []) > 0)
                                        <ul class="grid grid-cols-2 gap-1 rounded-lg bg-slate-50 p-3 text-xs ring-1 ring-slate-200">
                                            @foreach ($enrollment['recovery_codes'] as $code)
                                                <li class="font-mono text-slate-700">{{ $code }}</li>
                                            @endforeach
                                        </ul>
                                    ...
                                    @else
                                        <p class="rounded-lg bg-amber-50 p-3 text-xs text-amber-700 ring-1 ring-amber-200">
                            ...
                            <div class="border-t border-slate-100 pt-4">
                                <h3 class="mb-2 text-sm font-medium text-slate-900">Passo 3 — Confirme o primeiro código</h3>
            ...
            @else
                <div class="mt-5 border-t border-slate-100 pt-5">
                    <h3 class="text-sm font-medium text-slate-900">Desativar</h3>
